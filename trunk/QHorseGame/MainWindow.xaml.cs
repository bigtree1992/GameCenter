...
namespace QGameCenterLogic
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private UseRadioChoseGameLogic m_ChoseGame;
        private AutoScreenChangeLogic m_AutoScreenSyna;
        ...
        public MainWindow()
        {
            ...
            //var useCoin = false;
            //if(m_GameCenterConfig.UseCoin == 1)
            //{
            //    useCoin = true;
            //}
            //else
            ...
            //    useCoin = false; 
            //}
            //var useCoin = (bool)this.IsCoin.IsChecked;
            m_StartGameLogic.SetIsUseCoin(true);
            m_CheckCoinLogic.SetIsUseCoin(true);
           
            //开始切换屏幕逻辑
            m_AutoScreenSyna = new AutoScreenChangeLogic(this.GameImage, 3, m_ClientData, m_Server, m_ServerConfig,m_GameCenterConfig, this);
            m_AutoScreenSyna.Start();
            m_AutoScreenSyna.SetCheckIsStartGameIamge(startedShowImages);
            //成功开始游戏的回调
            m_StartGameLogic.OnIsStartOrCloseGame += (isStartGame) => {
                m_AutoScreenSyna.SetStartGame(isStartGame);
                this.Dispatcher.Invoke(() => {
                    Full.IsEnabled = !isStartGame;
                    Condensed.IsEnabled = !isStartGame;
            ...
            var radiobuttons = new RadioButton[2];
            radiobuttons[0] = this.Condensed;
            radiobuttons[1] = this.Full;
            m_ChoseGame = new UseRadioChoseGameLogic(m_GameData, radiobuttons, (index) => {
                m_CheckCoinLogic.SetCurIndex(index);
                m_StartGameLogic.SetCurGameIndex(index);
            }, (string content) => { Message.ShowMessage(content); });
        }
        ...
        private void OnWindowClose(object sender, EventArgs e)
        {
            ...
            if (m_AutoScreenSyna != null)
            {
                m_AutoScreenSyna.Stop();
                m_AutoScreenSyna = null;
            }
